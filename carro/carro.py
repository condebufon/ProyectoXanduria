class Carro:
    def __init__(self, request):
        self.request = (
            request  # Almacena el objeto request que contiene la petición HTTP.
        )
        self.session = (
            request.session
        )  # Accede a la sesión del usuario a través del request.
        carro = self.session.get("carro")  # Intenta obtener el carrito de la sesión
        if not carro:
            carro = self.session["carro"] = (
                {}
            )  # Si no existe el carrito en la sesión, lo crea como un diccionario vacío
        # else:
        self.carro = carro  # Si ya existe el carrito, lo asigna a la variable de instancia self.carro

    def agregar(self, producto):
        if (
            str(producto.id) not in self.carro.keys()
        ):  # Convierte el id del producto en una cadena y verifica si esa clave ya existe en el carrito
            ...
            for (
                ...
                self.carro.items()
            ):  # Se recorre el diccionario self.carro para buscar el producto cuyo ID coincide
                if key == str(producto.id):
                    value["cantidad"] = float(value["cantidad"]) + 1
    ...
    def guardar_carro(
        self,
    ):  # Después de agregar o actualizar el producto, llama a la función guardar_carro para guardar el carrito en la sesión.
        self.session["carro"] = (
            self.carro
        )  # Almacena el diccionario self.carro en la sesión del usuario bajo la clave "carro". Esto permite que el carrito persista entre solicitudes HTTP.
    ...
    def restar_producto(self, producto):
        for key, value in self.carro.items():  # Recorrer el carrito de compras
            if key == str(producto.id):  # Buscar el producto en el carrito
                value["precio"] = float(value["precio"]) - float(
                    producto.precio
                )  # Reducir el precio del producto en el carrito
                value["cantidad"] = (
                    value["cantidad"] - 1
                )  # Reducir la cantidad del producto en el carrito
                if (
                    value["cantidad"] < 1
                ):  # Eliminar el producto si su cantidad es menor que 1
    ...
    def total_precio(self):
        ...
        for item in self.carro.values():
            total += float(item["precio"])  # Sumar el precio de cada producto
        return total
